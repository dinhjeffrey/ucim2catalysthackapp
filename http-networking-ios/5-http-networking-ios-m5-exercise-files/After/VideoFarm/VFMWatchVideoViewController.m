...
- (void)configureView
{
    if (self.isViewLoaded) {
        ...
        // check to see if we're already downloading this movie
        if ([[VFMAPIClient sharedInstance] isDownloadingMovieFromVideo:self.video] ||
            [[VFMAPIClient sharedInstance] isDownloadingThumbnailImageFromVideo:self.video]) {
            self.downloadButton.enabled = NO;
        }
        else {
            // check to see if local version already exists
            NSFileManager *fm = [NSFileManager defaultManager];
            NSURL *localVideoURL = [[self.video movieURL] localDownloadsFilesystemURL];
            if ([fm fileExistsAtPath:localVideoURL.path] || self.download != nil) {
                self.downloadButton.enabled = NO;
            }
            else {
...
- (IBAction)didTapDownload:(id)sender
{
    ...
    [UIView animateWithDuration:0.5
                     ...
                     animations:^{
                         ...
                         [[VFMAPIClient sharedInstance] downloadMovieAndThumbnailForVideo:self.video];
