 ...
 */
static NSString * const CCVUserAPIKeyKey = @"APIKey";
...
+ (CCVChatcaveService *)sharedInstance
{
    ...
    dispatch_once(&onceToken, ^{
        SharedInstance = [[CCVChatcaveService_NSURLSession alloc] init];
    });
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        NSError *error = nil;
        NSData *APIKeyData = [CCVKeychain secretForKey:CCVUserAPIKeyKey error:&error];
        if (APIKeyData) {
...
- (void)persistServerRootAndUserIdentifier
{
    ...
    if (self.currentUser.APIKey) {
        ...
        if (! [CCVKeychain storeSecret:APIKeyData forKey:CCVUserAPIKeyKey error:&error]) {
            NSLog(@"ERROR: Unable to persist API key to the keychain: %@", error);
        }
    }
    // We're clearing the key out of the keychain
    else {
        if (! [CCVKeychain deleteSecretForKey:CCVUserAPIKeyKey error:&error]) {
